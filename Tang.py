...
def open_Tang():
    ...
    def them_tang():
        ...
        try:
            ...
            cur.execute("SELECT COUNT(*) FROM Tang WHERE Tang = ?", (tang,))

            if cur.fetchone()[0] > 0:
                messagebox.showwarning("Trùng lập", f"Tầng {tang} đã tồn tại")
                return

            cur.execute("Insert into Tang (Tang) VALUES (?)", (tang))
            conn.commit()
            load_data()
            clear_input()
    ...
    def xoa_tang():
        ...
        if not selected:
            messagebox.showwarning("Chưa chọn", "Hãy chọn 1 dòng để xoá")
            return
        item_id = selected[0]
        values = tree.item(item_id, "values")  # values là tuple
        tang = values[0]
        ...
        try:
            cur.execute("DELETE FROM Tang where Tang=?", (tang,))
            conn.commit()
            load_data()
    ...
    def sua_tang():
        ...
        if not selected:
            ...
            return
        item_id = selected[0]
        tang_cu = tree.item(item_id, "values")[0]  # giá trị đầu tiên
        entry_st.delete(0, END)
        entry_st.insert(0, str(tang_cu))
    ...
    def luu_tang():
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Chưa chọn", "Hãy chọn tầng để lưu")
            return
        item_id = selected[0]
        old_tang = tree.item(item_id, "values")[0]
        tang = entry_st.get()
        ...
        try:
            cur.execute("""UPDATE Tang SET Tang=? where Tang=?""", (tang, old_tang))
            conn.commit()
            load_data()
            clear_input()
