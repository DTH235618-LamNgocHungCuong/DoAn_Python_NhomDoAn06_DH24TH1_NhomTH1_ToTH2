...
def mo_menu(quyen):
   def center_window(win, w=400, h=250):
   ...
   rootM.minsize(width = 400, height = 250)
   center_window(rootM)
   ...
   Button(aButton, text="Phòng", font=("Times New Roman", 14), width=8, command=lambda: open_Phong()).grid(row=1,column=0)
   Button(aButton, text="Khách Hàng", font=("Times New Roman", 14), width=8, command=lambda: open_KhachHang()).grid(row=1, column=1)
   ...
   Button(aButton, text="Nhân Viên", font=("Times New Roman", 14), width=8, command=lambda: open_NhanVien()).grid(row=1,column=2)
